# bartlett_analysis_v17.py
# Restructured from bartlett_analysis_v16_1.py — Phase 2 of HANDOFF_STATE.md
# =========================================================


# # Bartlett Fellowship — Data Center Flexibility as Resource Adequacy
# ## Analysis Notebook (v17)
#
...
# |------|---------|-------------------|
# | **0** | **Setup & Parameters** | Shared constants, ten-parameter cascade, empirical destination data |
...
# |-----------|-------|-----------|
# | Cascade product (central) | 0.0467 | 10-param model (D1=0.99, D2-D5 decomposed) |
# | Commitment depth (inference-dominant) | 21.0% | Cascade + DVFS (v16.1 corrected formula) |
# | DVFS-only floor (facility) | 17.5% | Colangelo et al. 2026 (25% GPU x 0.70 S1) |
# | DR ELCC | 92% | PJM class rating (exogenous) |
# | BRA price | $333.44/MW-day | PJM 2027/28 |
# | Primary uncertainty | D2, D5 (Phase 4 surface) | Sensitivity surface pending |
#
# ### v17 Changes from v16.1
#
# 1. Parameter restructure: 7-parameter cascade -> 10-parameter explicit decomposition.
#    Old D2 (destination headroom, 0.85) decomposed into:
#      D2 (utilization headroom, 0.33), D3 (hardware compat, 0.88),
#      D4 (inference workload share, 0.50), D5 (pre-staging readiness, 0.65).
#    Old D3 (0.97) included pre-staging implicitly. New D3 is hardware-only (0.88)
#    and pre-staging is explicit in D5. See CHANGELOG.md and
#    compatible_fraction_reference.md for rationale.
#
# 2. Corrected Colangelo formula propagated to ALL locations (Parts 2.1, 2.2).
#    v16.1 had the fix in Part 0 only; Parts 2.1 and 2.2 still used the old
#    formula commit = spatial + DVFS * (1 - spatial). Now all locations use:
#    COMMITMENT_DEPTH = spatial + FLEX_FRAC * (S1 - spatial).
#
# ### Headline Results (from Parts 3, 5)
#
# | Metric | DVFS Only | DVFS + Spatial |
# |--------|-----------|---------------|
# | Commitment depth | 17.5% | ~21% |
# | See Parts 3, 5 for MW and value figures (regenerated under v17) |
...
# - Cell 4: v4 empirical data loading
...
# CELL 5
# ======================================================================
# Cell 0-2: SHARED PARAMETERS — Economic & Physical (v16)
# ===========================================================
...
# ### 0.3 Seven-Parameter Cascade (Pillar 1)
# 
...
# 
# **Source-side:** P1 (workload candidacy) × P3 (execution feasibility)  
# **Destination-side:** Cross-BA (stress decorrelation) × H (headroom)  
# **Execution:** P2 (completion) × P4 (hardware) × P5 (bandwidth)  
# 
# Product = effective spatial fraction = share of facility load that 
# disappears from the source node during dispatch. Central: **0.390**.


# ======================================================================
# CELL 7
# ======================================================================
# Cell 0-3: SEVEN-PARAMETER CASCADE — Pillar 1 (v16)
# ===========================================================
# Effective Spatial Fraction = S1 × S2 × D1 × D2 × D3 × E1 × E2
#
...
#
# v17: 10-PARAMETER CASCADE. Restructured from 7/8-parameter formulation.
# Old D2 (destination headroom, 0.85) decomposed into D2 (utilization
# headroom), D3 (hardware compat without pre-staging), D4 (inference
# workload share at destination), D5 (pre-staging / operational readiness).
# Old D3 (0.97) included pre-staging implicitly — now separated into
# D3 (hardware only, 0.88) and D5 (pre-staging, 0.65).
# See CHANGELOG.md, compatible_fraction_reference.md, PHASE2_PLAN.md.
#
# SOURCE QUALITY:
#   S1, E2:           [STRUCTURAL / DATA] — well-grounded
#   D1:               [DATA] — 35,060 hrs DA LMP empirical (Pillar 2)
#   S2b, D3, E1:      [ESTIMATED] — moderate grounding
#   S2a:              [ESTIMATED-WEAK] — no published shiftability measurement
...
CASCADE_S2a    = 0.80    # [ESTIMATED-WEAK] Data locality — frac without geographic pinning
CASCADE_S2b    = 0.90    # [ESTIMATED] Operational feasibility given advance notice
...
CASCADE_E1     = 0.997   # [ESTIMATED] Migration completion (cloud LB SLAs, conservative)
CASCADE_E2     = 0.995   # [ESTIMATED] WAN bandwidth (orders-of-magnitude argument)

# --- Cascade product (10-parameter) ---
EFFECTIVE_SPATIAL_FRAC = (CASCADE_S1 * CASCADE_S2a * CASCADE_S2b *
...
CASCADE_RANGES = {
    ...
    'S2a': (0.60,  0.80,  0.95),    # WEAK — no published shiftability measurement
    'S2b': (0.85,  0.90,  0.95),    # Moderate-strong (Azure trace: seconds vs minutes)
...
_cascade_con = (_con['S1'] * _con['S2a'] * _con['S2b'] *
                _con['D1'] * _con['D2'] * _con['D3'] *
                _con['D4'] * _con['D5'] *
                _con['E1'] * _con['E2'])
_cascade_opt = (_opt['S1'] * _opt['S2a'] * _opt['S2b'] *
...
print(f"    S2a (data locality):   {CASCADE_S2a:.2f}   [ESTIMATED-WEAK]")
print(f"    S2b (timing):          {CASCADE_S2b:.2f}   [ESTIMATED]")
...
# the companion stress correlation notebook (v4). These feed the energy 
...
# CELL 9
# ======================================================================
# Cell 0-3: EMPIRICAL DESTINATION DATA — v4 Stress Correlation Results
# ===========================================================
# Loads empirical destination LMPs and coincidence factors from the
# companion Cross_BA_Stress_Correlation_v4 notebook.
#
...
#
# DEPENDENCY: Cross_BA_Stress_Correlation_v4.ipynb must be run first
#             to generate stress_correlation_results.json
# ===========================================================

import json as _json

_V4_RESULTS_PATH = r'C:\Users\dunla\OneDrive\Documents\Bartlett Fellowship\Demand Response Direction\1_Working Version\stress_correlation_results.json'

try:
    with open(_V4_RESULTS_PATH) as _f:
        _v4 = _json.load(_f)

    # Capacity-weighted mean destination LMP during ComEd top-50 stress hours
    DESTINATION_LMP_CRISIS = _v4['empirical_destination_lmps']['capacity_weighted_mean']

    # Per-interconnection destination LMPs
    _dest_zones = _v4['per_zone']
    _ic_lmps = {}
    ...
    EMPIRICAL_CF = _v4['headline']['capacity_weighted_overlap_pct'] / 100

    DESTINATION_LMP_NORMAL = 40.0  # [ESTIMATED] non-crisis average

    _V4_LOADED = True
    print(f'✓ v4 empirical values loaded:')
    ...
    _V4_LOADED = False
    print(f'⚠ WARNING: v4 results not found.')
    print(f'  Path: {_V4_RESULTS_PATH}')
    print(f'  Run Cross_BA_Stress_Correlation_v4.ipynb first.')
...
# CELL 12
# ======================================================================
# Cell 1-1: ComEd DA LMP LANDSCAPE (v16)
# ===========================================================
...
# CELL 14
# ======================================================================
# Cell 1-2: STRESS EVENT IDENTIFICATION (v16)
# ===========================================================
...
# CELL 16
# ======================================================================
# Cell 1-3: STRESS EVENT CHARACTERIZATION & HEADLINE FINDINGS (v16)
# ===========================================================
...
# CELL 19
# ======================================================================
# Cell 2-1: CASCADE MONTE CARLO VARIANCE DECOMPOSITION (v16)
# ===========================================================
# Monte Carlo sensitivity analysis for the seven-parameter cascade.
# Produces:
...
samples = {
    'S2a': np.random.uniform(0.60, 0.95, N),      # WEAK — no published shiftability measurement
    'S2b': np.random.uniform(0.85, 0.95, N),       # Moderate-strong
...
grounding = {
    'S2a': 'WEAK', 'S2b': 'Moderate', 'D1': 'STRONG',
...
print(f"    S2a, D4 (the weakest-grounded parameters with widest ranges).")
...
# CELL 21
# ======================================================================
# Cell 2-2: CASCADE SCENARIO SUMMARY (v16)
# ===========================================================
...
CASCADE_SCENARIOS = {
    'Conservative': _cascade_con,       # 0.281
    'Central':      EFFECTIVE_SPATIAL_FRAC,  # 0.390
    'Optimistic':   _cascade_opt,       # 0.511
}
...
for param, (lo, mid, hi) in CASCADE_RANGES.items():
    ...
    vals_lo[param] = lo
    prod_lo = (vals_lo['S1'] * vals_lo['S2a'] * vals_lo['S2b'] *
    ...
    vals_hi[param] = hi
    prod_hi = (vals_hi['S1'] * vals_hi['S2a'] * vals_hi['S2b'] *
...
print(f"    D2 (utilization headroom), D5 (pre-staging), S2a (data locality),")
...
# CELL 24
# ======================================================================
# Cell 3-1: MECHANISM PROFILES & PORTFOLIO DEFINITIONS (v16)
# ===========================================================
...
MECHANISM_PROFILES = {
    ...
    'Spatial Migration': {
        'max_depth': EFFECTIVE_SPATIAL_FRAC,  # 7-param cascade product
        'max_duration': None,             # Indefinite
...
PORTFOLIOS = {
    ...
    'DVFS + Spatial': {
        ...
        'desc': 'KEY PORTFOLIO. Spatial doubles commitment. Requires orchestration layer.'
    },
    'Full Stack': {
        'depth': None,
        'duration': None,
...
# CELL 26
# ======================================================================
# Cell 3-2: COMMITMENT OPTIMIZATION (v16)
# ===========================================================
...
# CELL 28
# ======================================================================
# Cell 3-3: ELECTION MECHANISM & AVOIDED INSTALLED CAPACITY (v16)
# ===========================================================
...
print("ELECTION-BASED SPATIAL CAPACITY & AVOIDED INSTALLED CAPACITY (v13.1)")
print("=" * 100)
...
# CELL 29
# ======================================================================
# Cell 3-3b: INFERENCE READINESS BREAK-EVEN (v15.1)
# ===========================================================
# Inference routing has zero per-event friction. The only cost
# is annual readiness (pre-staging, serving stack, bandwidth).
# This cell computes: how high would readiness cost need to be
# to make inference routing uneconomic?
#
# INPUTS: EFFECTIVE_SPATIAL_FRAC, BRA_2027_28_PRICE, DR_ELCC,
#         PLANNING_RESERVE_MARGIN, E3_CT_LEVELIZED_COST (from Parts 0, 3)
# OUTPUTS: Print-only (break-even analysis)
# ===========================================================
# ─────────────────────────────────────────────────────────────
# PER-FACILITY CAPACITY REVENUE — INFERENCE ROUTING (v15.1)
# ─────────────────────────────────────────────────────────────
...
# CELL 31
# ======================================================================
# Cell 3-4: E3 COUNTERFACTUAL — AVOIDED CT COST (v16)
# ===========================================================
...
print(f'\n  INTERPRETATION:')
print(f'    Spatial doubles commitment depth ({DVFS_COMMITMENT_FRAC:.0%} → {OPTIMAL_COMMITMENT_FRAC:.0%}),')
print(f'    doubling avoided CT build. IX queue acceleration (Part 5)')
print(f'    dominates the investment decision; capacity revenue is the')
print(f'    compliance hook, not the primary incentive.')
...
# CELL 34
# ======================================================================
# Cell 4-1: ENERGY ARBITRAGE & GROUNDED VALUE STACK (v16)
# ===========================================================
...
# CELL 36
# ======================================================================
# Cell 4-2: THREE-PRONG ENERGY ECONOMICS (v16)
# ===========================================================
# Energy-only value of each flexibility mechanism, using empirical
# destination LMPs from v4. This establishes that spatial migration
...
spatial_load_drop = EFFECTIVE_SPATIAL_FRAC  # 7-param cascade
...
# CELL 38
# ======================================================================
# Cell 4-3: SPATIAL BREAK-EVEN — FORWARD PROJECTION (v16)
# ===========================================================
...
# CELL 40
# ======================================================================
# Cell 4-4: PORTFOLIO INTERACTION EFFECTS (v16)
# ===========================================================
...
# CELL 43
# ======================================================================
# Cell 5-1: IX QUEUE NPV — BEHAVIORAL INCENTIVE (v16)
# ===========================================================
...
# CELL 45
# ======================================================================
# Cell 5-2: FORWARD ENERGY ARBITRAGE — MONTE CARLO PROJECTION (v16)
# ===========================================================
...
# CELL 48
# ======================================================================
# Cell 6-1: CONSOLIDATED RESULTS (v16)
# ===========================================================
...
print('╔' + '═'*88 + '╗')
print('║' + ' CONSOLIDATED RESULTS — BARTLETT FELLOWSHIP v15 '.center(88) + '║')
print('╚' + '═'*88 + '╝')
...
# ── KEY FINDINGS ─────────────────────────────────────────────
# v17 Phase 3: per-GW framing. Reference cases at 1 GW and 10 GW.
# Headline is commitment depth as a function of fleet size, not
# a single fleet-size number.

print(f'\n{"="*70}')
print(f'KEY FINDINGS (v17 per-GW framing):')
...
print(f'  Spatial uplift:               {OPTIMAL_COMMITMENT_FRAC/DVFS_COMMITMENT_FRAC:.1f}x more accredited capacity')
print(f'  ── 1 GW reference (individual facility scale) ──')
...
print(f'  Spatial break-even:           2028 (central), 2030 (conservative)')
...
# CELL 50
# ======================================================================
# Cell 6-2: VALIDATION CHECKS (v16)
# ===========================================================
...
_check(abs(EFFECTIVE_SPATIAL_FRAC - 0.0467) < 0.005,
       "Cascade product ≈ 0.0467 (v17 10-parameter)",
       f"actual: {EFFECTIVE_SPATIAL_FRAC:.4f}")

_check(abs(COMMITMENT_DEPTH - 0.210) < 0.01,
       "Commitment depth ≈ 21.0% (v17: DVFS on shiftable residual, 10-param cascade)",
...
_check(abs(CASCADE_D2 - 0.33) < 0.001,
       "CASCADE_D2 = 0.33 (v17: utilization headroom)",
       f"actual: {CASCADE_D2}")

_check(abs(CASCADE_D3 - 0.88) < 0.001,
       "CASCADE_D3 = 0.88 (v17: HW compat without pre-staging)",
       f"actual: {CASCADE_D3}")

_check(abs(CASCADE_D4 - 0.50) < 0.001,
       "CASCADE_D4 = 0.50 (v17: inference workload share)",
       f"actual: {CASCADE_D4}")

_check(abs(CASCADE_D5 - 0.65) < 0.001,
       "CASCADE_D5 = 0.65 (v17: pre-staging readiness)",
       f"actual: {CASCADE_D5}")
...
# v17 Phase 3: validate at both reference fleet sizes
# 10 GW: ~21% commitment × 10 GW × 92% ELCC ≈ 1,932 MW
_check(1500 < REF_ACCREDITED_MW_10GW < 2500,
       "Accredited MW (10GW ref) in [1500, 2500] (v17 range)",
...
_check(150 < REF_ACCREDITED_MW_1GW < 250,
       "Accredited MW (1GW ref) in [150, 250] (v17 range)",
...
if 'election_results' in dir() and 10.0 in election_results:
    ...
    # v17: lower commitment depth means lower avoided capacity
    _check(1000 < _r['total_avoided'] < 4000,
           "Total avoided (10GW ref) in [1000, 4000] (v17 range)",
...
if 'election_results' in dir() and 1.0 in election_results:
    ...
    _check(100 < _r1['total_avoided'] < 400,
           "Total avoided (1GW ref) in [100, 400] (v17 range)",
...
# v17: Cross-document checks are EXPECTED TO FAIL until Phase 5 reconciliation.
# Documents still contain stale v15/v16 numbers. These checks are intentionally
# updated to v17 targets so they will pass once documents are reconciled.
_v17_cascade_central = 0.0467
_v17_commitment_central = 0.210

_check(abs(EFFECTIVE_SPATIAL_FRAC - _v17_cascade_central) < 0.005,
       f"Cascade matches v17 target ({_v17_cascade_central})",
       f"notebook: {EFFECTIVE_SPATIAL_FRAC:.4f}")

_check(abs(OPTIMAL_COMMITMENT_FRAC - _v17_commitment_central) < 0.01,
       f"Commitment depth matches v17 target ({_v17_commitment_central})",
       f"notebook: {OPTIMAL_COMMITMENT_FRAC:.4f}")

_check(abs(HEADLINE_COMMITTED_MW_10GW - _pillar3_committed_mw) < 50,
       f"Committed MW matches Pillar 3 ({_pillar3_committed_mw:,})",
       f"notebook: {HEADLINE_COMMITTED_MW_10GW:,.0f}")

_check(abs(HEADLINE_ACCREDITED_MW_10GW - _pillar3_accredited_mw) < 50,
       f"Accredited MW matches Pillar 3 ({_pillar3_accredited_mw:,})",
       f"notebook: {HEADLINE_ACCREDITED_MW_10GW:,.0f}")


# ── v4 DATA LOADED ────────────────────────────────────────────

try:
    _check(_V4_LOADED == True,
           "v4 empirical data loaded (not fallback)",
           f"DESTINATION_LMP_CRISIS = ${DESTINATION_LMP_CRISIS:.1f}")
except NameError:
    _check(False, "v4 empirical data loaded", "_V4_LOADED not defined")
...
if _checks_passed == _checks_total:
    print("  ✓ All v13.1 validation checks passed")
else:
...
# CELL 52
# ======================================================================
# Cell 6-3: EXPORT RESULTS TO CSV (v16)
# ===========================================================
...
# CELL 53
# ======================================================================
# EXPERIMENTAL: CONTINUOUS INFERENCE ROUTING ARBITRAGE
# ===========================================================
import pandas as pd
import numpy as np

_BASE_V4 = r'C:\Users\dunla\OneDrive\Documents\Bartlett Fellowship\Demand Response Direction\1_Working Version'
_hourly = pd.read_parquet(os.path.join(_BASE_V4, 'hourly_zone_prices.parquet'))
_meta = pd.read_parquet(os.path.join(_BASE_V4, 'zone_metadata.parquet'))
...
_routable_frac = CASCADE_S1 * CASCADE_S2a

print('CONTINUOUS INFERENCE ROUTING ARBITRAGE')
print('=' * 85)
print(f'Source: {_source_col}')
print(f'Destinations: {len(_dest_cols)} zones')
print(f'Routable fraction: {_routable_frac:.1%} (S1 x S2a = {CASCADE_S1} x {CASCADE_S2a})')
...
# CELL 54
# ======================================================================
print([v for v in dir() if 'CASCADE' in v or 'S2' in v.upper()])


# ======================================================================
# CELL 55
# ======================================================================
# EXPERIMENTAL: ELASTICITY SENSITIVITY — WHAT DOES IT TAKE?
# ===========================================================
